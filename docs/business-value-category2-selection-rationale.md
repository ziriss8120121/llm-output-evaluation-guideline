...
| 候補 | 今回の扱い | 理由 / 対応先 |
|---|---|---|
| 目的・タスク適合 / Task Completion / Functional Appropriateness | **採用 / 統合** | 「目的・タスク適合」として採用。LLM機能固有の目的・期待価値への適合を扱う |
| Helpfulness | **採用 / 統合** | 単独区分にすると完全性・関連性・指示遵守等を包含しすぎるため、「目的・タスク適合」の考え方へ統合 |
| 完全性 / Completeness / Response Completeness | **採用** | 必要な情報・要素・論点が不足していないかを扱う |
| 関連性 / Relevance / Answer Relevancy | **採用** | 要求と関係のない情報に逸れていないかを扱う |
| 指示遵守 / Following Instructions / Instruction Following | **採用** | プロンプトで明示した条件・形式・制約等を扱う |
| 論理的一貫性 / Coherence | **採用 / 統合** | 「表現品質」の下位観点として扱う |
| 自然さ・流暢性 / Fluency | **採用 / 統合** | 「表現品質」の下位観点として扱う |
| スタイル・トーン | **採用 / 統合** | 「表現品質」の下位観点として扱う |
| 簡潔性・冗長性 / Verbosity | **採用 / 統合** | 「表現品質」の下位観点として扱う |
| 正確性 / Correctness / Accuracy | **ビジネス価値から除外** | 固定済み事業リスク「誤情報・誤誘導」で評価するため。二重評価防止 |
| Functional Correctness | **ビジネス価値から除外** | 上記と同様。一般的には重要な品質軸だが、本ガイドラインでは事業リスク側へ寄せる |
| 根拠への忠実性 / Faithfulness | **ビジネス価値から除外** | 入力・参照情報にない内容の生成は「誤情報・誤誘導」で評価するため |
...
| Customer Satisfaction | **独立区分にはしない** | 複数の品質要因を包含する総合的な指標であり、区分2として横並びにすると包含関係が大きくなるため |
...
| Refusal | **独立区分にはしない** | 必要な回答を不当に拒否した場合は「目的・タスク適合」または「指示遵守」で評価可能。安全上の適切な拒否は事業リスク側の設計と合わせて判断する |
